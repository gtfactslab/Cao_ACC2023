...
safe_rect = [-5, -30, -2, -30, -2*pi, -2*pi, 5, 30, 0, 30, 2*pi, 2*pi];
...
goal_pt = [0, 0, 10, 0, 0, 0];
goal_bound = [5, 8, 1, 8, pi, pi];
...
u_max = [40, 2*pi];
safety_controller = @(x, u) sixquad_safety_control(x, u);
actual_system = @sixquad_windy;


if ~all(goal_rect(7:end) >= goal_rect(1:6))
    error('goal rect defined incorrectly!')
elseif ~all(safe_rect(7:end) >= safe_rect(1:6))
...
sampling_points_U = disturbance_y(sampling_points_Y);
sampling_points_V = disturbance_z(sampling_points_Y);
...
obs_z = [];
for i = 1:numel(z_vec)
    obs_y = [obs_y; z_vec(i), disturbance_y(z_vec(i))];
    obs_z = [obs_z; z_vec(i), disturbance_z(z_vec(i))];
end
z_space = [-2:0.25:12];
...
% horizons
blocking_interval = 8; % number of timesteps to execute each control action
...
if write_video
    ...
    writer1 = VideoWriter('state_6quadfull', 'UNCOMPRESSED AVI');
    writer1.FrameRate = 1;
...
for i = 1:100
    ...
    if numel(obs_y(1, 1:end-1)) == 1
        plot_GP(2, obs_y, z_space, "y", @disturbance_y) % 1d
        plot_GP(3, obs_z, z_space, "z", @disturbance_z) % 1d
    elseif numel(obs_y(1, 1:end-1)) == 2
        grid_pts = plot_GP3D(2, 1, obs_y, x1_space, x2_space, disturbance_y); %2d
%         nlobj.Model.StateFcn = @(x, u) kin_bike_icystate_emb_mpc(x, u, grid_pts);
    ...
    if ~path_to_goal_found
        ...
        if ~exitFlag % if reaching goal infeasible, try safety
            ...
            nlobj.Optimization.CustomCostFcn = @(X,U, e, data) -1*altitude(X, U, e, data, blocking_interval);
...
%     exitFlag = all(leq_constraints(info.Xopt, info.MVopt, 0, 0) <= 0);
    ...
    plot(x_step(:,1), x_step(:, 3), step_color);
    scatter(x_step(1,1), x_step(1, 3), 'kx')
    ...
    if ~isempty(storedXopt)
        ...
        plot(storedXopt(step:end,1), storedXopt(step:end, 3), 'k:');
        plot(storedXopt(step:end,7), storedXopt(step:end, 9), 'k:');
    end
    ...
    if debug
        ...
        plot(info.Xopt(step:end,1), info.Xopt(step:end, 3), 'k:');
        plot(info.Xopt(step:end,7), info.Xopt(step:end, 9), 'k:');
    end
...
%     axis([-1, 1, -2, 2])
    ...
    % update obs
    obs_y = [obs_y; cur_x(3), disturbance_y(cur_x(3))]; % 1d
    obs_z = [obs_z; cur_x(3), disturbance_z(cur_x(3))]; % 1d
